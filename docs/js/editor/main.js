...
async function main() {
  ...
  // Shared helper: apply a parsed config JSON object into the editor
  async function loadConfigIntoEditor(json) {
    loadConfig(json);
  ...
  // Wire preset dropdown (auto-loads on selection)
  ...
  presetSelect.addEventListener('change', async () => {
    ...
    try {
      ...
      await loadConfigIntoEditor(json);
    } catch (err) {
  ...
  document.getElementById('file-input').addEventListener('change', async e => {
    ...
    try {
      await loadConfigIntoEditor(JSON.parse(text));
    } catch (err) {
  ...
  // Load config sent from the simulation page ("Open Editor" button)
  const simConfigJson = localStorage.getItem('crystal_sim_current_config');
  if (simConfigJson) {
    localStorage.removeItem('crystal_sim_current_config');
    try {
      await loadConfigIntoEditor(JSON.parse(simConfigJson));
    } catch (err) {
      console.warn('Failed to load config from simulation:', err);
    }
  }
