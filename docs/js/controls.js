...
export function initControls(simAPI, renderer, config, onReset) {
  ...
  function applyConfig(cfg) {
    ...
    tempSlider.value = cfg.temperature ?? 1.0;
    tempVal.textContent = Number(cfg.temperature ?? 1.0).toFixed(2);
    renderer.paused = true;
    pauseBtn.textContent = 'Run';
    buildMuSliders(cfg);
    onReset(cfg);
  ...
  loadConfigBtn.addEventListener('click', async () => {
    const url = configSelect.value;
    if (url === 'editor') return;  // "From Editor" is display-only
    try {
      const cfg = await fetch(url).then(r => {
